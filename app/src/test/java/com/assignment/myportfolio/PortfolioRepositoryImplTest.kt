...
import com.assignment.myportfolio.data.remote.PortfolioData
import com.assignment.myportfolio.data.remote.PortfolioResponse
import com.assignment.myportfolio.data.remote.PortfolioService
import com.assignment.myportfolio.data.remote.Holding
...
class PortfolioRepositoryImplTest {
    ...
    @Test
    fun `Given empty cache and network success, When getHoldings(forceRefresh=true), Then data is cached and returned`() =
        runTest {
            // Given
            ...
            coEvery { dao.getHoldings(any()) } returnsMany listOf(
                emptyList(),
                listOf(
                    HoldingDBEntity(
                        0,
            ...
            // When
            val repo = PortfolioRepositoryImpl(service, db, dao, json)
            val result = repo.getHoldings(forceRefresh = true)
